...
for K = 4 : 10
    ...
    for i = 1 : 100
        ...
        [Fv1, Fvs1, upsilonk_hat1, upsilon_hat1] = QCS(X, Z1);
        [Fv2, Fvs2, upsilonk_hat2, upsilon_hat2] = QCS(X, Z2);
        [Fv3, Fvs3, upsilonk_hat3, upsilon_hat3] = QCS(X, Z3);
        [Fv4, Fvs4, upsilonk_hat4, upsilon_hat4] = QCS(X, Z4);
        [Fv5, Fvs5, upsilonk_hat5, upsilon_hat5] = QCS(X, Z5);
        [Fv6, Fvs6, upsilonk_hat6, upsilon_hat6] = QCS(X, Z6);

        A_num1(K - 3, i) = find(ismember(Fvs1, [1 2 101]), 1, 'last');
        A_num2(K - 3, i) = find(ismember(Fvs2, [3 4 102]), 1, 'last');
        A_num3(K - 3, i) = find(ismember(Fvs3, [5 6 103]), 1, 'last');
        A_num4(K - 3, i) = find(ismember(Fvs4, [7 8 104]), 1, 'last');
        A_num5(K - 3, i) = find(ismember(Fvs5, [9 10 105]), 1, 'last');
        A_num6(K - 3, i) = find(ismember(Fvs6, [11 12 106]), 1, 'last');
